...
main:
    #mutam codul pentru system call in eax
   movl $0xc0, %eax
   
    #mutam adresa la care vrem sa fie alocata memoria
    #punem 0 pentru a lasa sistemul de operare sa decida
   xorl %ebx, %ebx
   
    #alocam 50000 de bytes de memorie
   movl $50000, %ecx
   
    #avem nevoie de flag-urile PROT_READ si PROT_WRITE
    #acestea sunt 0x1 respectiv 0x2
    #facem OR intre ele sa le combinam si va da 0x3
   movl $0x3, %edx
   
    #mutam flag- urile in esi
    #folosim MAP_PRIVATE care este 0x02, deoarece dorim ca memoria sa fie accesata doar de procesul actual
    #folosim  MAP_ANONYMOUS care este 0x20, deoarece nu avem un fisier asociat memoriei
    #folosim OR sa le combinam
   movl $0x02, %esi
   or $0x20, %esi
   
    #nu avem un fisier asociat deci punem -1 in file descriptor
   movl $-1, %edi
   
    #nu avem nevoie de offset deci punem 0
   movl $0,%ebp
   
    #dam call
   int $0x80 
    
    #pointer ul catre memoria alocata va fi returnat in %eax
    movl %eax, m1
   
   #repetam procesul pt celelalte 2 matrici
...
read_exit: 
        movl m1, %esi  #in esi vom avea matricea
...
et_exit:
 ...
 #dealocam memoria folosind munmap
 
 #codul pentru system call-ul lui munmap mutat in %eax
        movl $0x5b, %eax
 #mutam adresa matricei in %ebx
        movl m1, %ebx
 #mutam lungimea matricei in %ecx
        movl $50000, %ecx             
    int $0x80
    
  #repetam pt celelalte matrici
